...
public class JobPostActivityController {
    ...
    public String searchJobs(Model model,
        ...
        if(days30) {
            searhDate = LocalDate.now().minusDays(30) ;
        } else if (days7) {
            searhDate = LocalDate.now().minusDays(7) ;
        }
        else {
        ...
        if(!dateSearchFlag && !remote && !type && !StringUtils.hasText(job)
            ...
            jobPost = jobPostActivityService.search(job, location,
                    Arrays.asList(partTime,fullTime,freelance), Arrays.asList(remoteOnly,officeOnly, partialRemote),
                    searhDate) ;
        }
        Object currentUserProfile = usersService.getCurrentUserProfile();
        System.out.println("aa:"+currentUserProfile);
